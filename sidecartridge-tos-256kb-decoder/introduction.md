...
This chapter provides a concise overview of the SidecarTridge TOS 256KB Decoder, explaining its purpose, features, and benefits for Mega ST computers.

## Overview

The **256KB Decoder** is an add-on board for the SidecarTridge TOS emulator that enables decoding of 256KB ROMs used in the Atari STe and Mega STe computers, bringing this functionality to the Atari Mega ST series. **This new 100% solderless solution plugs into the MEGABUS connector of the Atari Mega ST computer and is intended for SidecarTridge TOS Emulator carrier board revision 4 and higher**, performing two key functions:
...
Combined with a custom version of the SidecarTridge TOS emulator hardware, a Mega ST computer can transparently decode both 192KB and 256KB ROMs.

{: .warning }
The current installation process documented in this section only applies to Atari Mega ST computers with the SidecarTridge TOS Emulator carrier board revision 4 and higher. Older TOS Emulator carrier board revisions are covered by the [discontinued hardware installation page](/sidecartridge-tos-256kb-decoder/hardware-installation-discontinued/) for reference only.
...
The SidecarTridge TOS 256KB Decoder is an extension of the [SidecarTridge TOS emulator](/sidecartridge-tos/) project. It requires a compatible Mega ST version of the SidecarTridge TOS emulator carrier board to function, currently revision 4 and higher, which connects to the 256KB Decoder board.
...
2. **SidecarTridge Mega ST Models Carrier Board for 256KB Decoder**: Designed to fit seamlessly into various Mega ST models, the carrier board includes a multiplexer. This allows the SidecarTridge TOS emulator to select between 192KB and 256KB ROMs, enabling transparent decoding of both.

3. **256KB Decoder Board**: Plugged into the MEGABUS connector, this add-on board decodes the 256KB ROMs used in the Atari STe and Mega STe computers when installed in the Mega ST series. It supplies the necessary signals to the SidecarTridge TOS emulator.

4. **6-Pin flat cable**: This cable links the 256KB Decoder board to the SidecarTridge TOS emulator carrier board.
